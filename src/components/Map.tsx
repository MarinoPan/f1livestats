...
export default function Map() {
  ...
  const positionData = useDataStore((state) => state["Position.z"]);
  ...
  // Estraiamo le posizioni più recenti da Position.z
  const positions =
    positionData?.Position?.[positionData.Position.length - 1]?.Entries || {};
  ...
  useEffect(() => {
    ...
    const cornerPositions: Corner[] = circuitData.corners.map((corner) => {
      ...
      const { x: lx, y: ly } = rotate(
        corner.trackPosition.x + labelDistance * Math.cos(rad(corner.angle)),
        corner.trackPosition.y + labelDistance * Math.sin(rad(corner.angle)),
        finalAngle,
        centerX,
        centerY
      );
  ...
  return (
    ...
    >
      ...
      {centerX && centerY && orderedDrivers && orderedDrivers.length > 0 && (
        <>
