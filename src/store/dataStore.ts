    ...
    (set, get) => ({
      ...
      update: (newData) => {
        set((state) => {
          ...
          // Gestione speciale per Position.z
          if ("Position.z" in newData) {
            // Aggiorniamo anche Positions con i dati di Position.z
            const positionData = newData["Position.z"];
            if (
              positionData &&
              positionData.Position &&
              positionData.Position.length > 0
            ) {
              const latestPosition =
                positionData.Position[positionData.Position.length - 1];
              if (latestPosition && latestPosition.Entries) {
                newData = {
                  ...newData,
                  Positions: latestPosition.Entries,
                };
              }
            }
          }
