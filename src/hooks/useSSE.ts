...
export function useSSE() {
  ...
  const processSSEData = useCallback(
    (event: SSEEvent) => {
      console.log("📥 Evento SSE ricevuto:", JSON.stringify(event, null, 2));
      ...
      if (eventId <= lastEventId.current) {
        console.log(`🔄 Dato ignorato (${channel}): ID evento obsoleto`);
        return;
      }
  ...
  const cleanupStore = useCallback(() => {
    ...
    // Filtriamo le proprietà che vogliamo escludere
    const stateWithoutUpdate = Object.entries(currentState)
      .filter(([key]) => key !== "update" && key !== "lastUpdateTime")
      .reduce((acc, [key, value]) => ({ ...acc, [key]: value }), {});

    const cleanedState = Object.entries(stateWithoutUpdate).reduce<
      Partial<Omit<StoreState, "update" | "lastUpdateTime">> & {
        "CarData.z"?: CarData | undefined;
        "Position.z"?: PositionZ | undefined;
      }
    >((acc, [key, value]) => {
      if (
        value &&
        typeof value === "object" &&
        "_timestamp" in value &&
        typeof value._timestamp === "number" &&
        value._timestamp < threshold
      ) {
        console.log(`🧹 Pulizia dati obsoleti per: ${key}`);
        // Per CarData.z e Position.z, usiamo undefined invece di null
        if (key === "CarData.z" || key === "Position.z") {
          return acc; // Non includiamo la proprietà affatto
        }
        return { ...acc, [key]: null };
      }
